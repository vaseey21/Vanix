...
| 03 | Home dashboard (Farm Owner) | Done — wordmark + working "All Farms (3)" farm-selector dropdown; 6 compact stat tiles in a 3×2 grid (Total Cattle / Unresolved alerts / Pending tasks / Pregnant·Heat combined / Approvals pending / Milking missed); Today/This-Week timeline (greenInk-consistent accents, no Vaccination Drive row) with "View all ›" → full Schedule page grouped Today/This Week/Later; "Needs attention" → Cows-in-heat / Cows-in-gestation rows (count pill + chevron → bottom-sheet cow list); Updates list with "View all ›" → full page. Home nav dot removed; Farms nav icon swapped to a barn glyph app-wide. Dark mode + hi/bho i18n done (runtime UI localizer scoped to the new page containers, en⇄hi/bho reversible). |
...
| 06 | Cow profile (vitals, cycle, yield) | Done — opened by tapping a cow in Farm-detail's cattle list. Full-bleed cow-photo header (name+belt, breed·age, gender·battery, status pill·temp, borderless download button, dark scrim). Tabs: Timeline / Overview / Milk Data / Vet Logs / Activity. Overview = 2×2 icon-less compact cards (Last/Next heat cycle, Last temp spike, calving count). Activity = duration dropdown (Today/Yesterday/This week/This month) + behaviour filter chips + 4 activity-status tiles (Rumination/Feeding/Standing/Resting) + rumination anomaly graph + day-scroller activity log. Timeline/Milk/Vet are placeholders. Dark mode + hi/bho i18n done (runtime UI localizer). **Add Cattle** (FAB → `#page-add-cattle`): back + "Add Cattle" (no subtext), Cow Details / Device Details tabs, photo upload, Name/Type/Breed/Gender/Age/Status (calf dropped, "Unknown" added), "+ Add cow history" → bottom sheet, Belt Number + auto-populating Node/Mac ID under Device Details, Cancel/Save. |
