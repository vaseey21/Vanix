...
| 11 | Account | Done — Account tab (all navs + home) opens the Settings page: profile row (James Redmark, Farm Owner → read-only Profile page w/ Request Account Deletion), grouped rows (Farm Management / Cattle Groups; Alerts & Contacts: Alert-sound toggle — alerts themselves are mandatory, no notification toggles — and Vet & Emergency Contacts; App: Language switcher (live, en/hi/bho) + Dark-mode toggle (moved here from the raw Account-tab tap); Legal: Privacy Policy + Terms pages; Support: Help & FAQs; Log Out). Vet & Emergency Contacts: vets onboard via confirmation link — Add Vet sends an invite and the vet card shows PENDING until accepted, DECLINED with a Resend-invite button if refused, CONFIRMED once accepted (demo confirm/decline buttons simulate the vet side); no primary/backup wording; Emergency Contacts are a separate section. All chrome localized (en/hi/bho); legal body text EN. |
