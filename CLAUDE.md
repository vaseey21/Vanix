...
| 07 | Milk Log | Done — cream hero, milk-scoped tinted banners (NO heat alerts here), date-grouped cards w/ coloured yield box + ✓/⏱, two-pane filter sheet (icon rail left w/ activebg pill + greenInk accent bar, bordered white option rows w/ radio circles → white check circle on greenInk selected fill, greenInk Apply CTA, green drag handle — shared design with the Events filter sheet), tap card → Edit/Delete action sheet, black FAB. Sessions: Morning + Evening only. "View complete summary" → analytics page: retained totals, breed filter, 8-week trend w/ tooltip, highest/lowest week tiles, top-5 cows + yield-by-breed bars (single-hue greenInk/greenDeep) |
