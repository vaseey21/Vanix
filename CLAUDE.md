...
| 03 | Home dashboard (Farm Owner) | Done — wordmark + working "All Farms (3)" farm-selector dropdown; 6 compact stat tiles in a 3×2 grid (Total Cattle / Unresolved alerts / Pending tasks / Pregnant·Heat combined / Approvals pending / Milking missed); Today/This-Week timeline (greenInk-consistent accents, no Vaccination Drive row) with "View all ›" → full Schedule page grouped Today/This Week/Later; "Needs attention" → Cows-in-heat / Cows-in-gestation rows (count pill + chevron → bottom-sheet cow list); Updates list with "View all ›" → full page. Home nav dot removed; Farms nav icon swapped to a barn glyph app-wide. Built English-first — home i18n + dark-mode polish pending. |
