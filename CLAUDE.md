...
| 04 | Farms list | Done — hero (Farms title + "Manage all your dairy farms" subtitle) + a period dropdown (Today / Last 7 days / Last 30 days) that drives 3 icon stat tiles (Total Farms / Total Cattle / Total Milk — Milk recomputes per period). Search + Swiggy-style filter dropdown (All / Healthy / Need Attention / Setup Required). "Your Farms" section: rich farm cards — status dot + farm-icon circle, name, manager · cattle count, location (pin), a CRITICAL/HEALTHY severity badge + chevron, and a row of 4 stat chips (Heat / Fever / Insemination / Milk Today). Setup farms render as a dashed row with a "Setup Farm" pill. No "Add Farm" — farms are provisioned by the distributor/admin. Tapping a card opens Farm Detail. |
| 05 | Farm detail (cow list) | Done — back chevron + farm name + manager, kebab menu top-right. 3 stat tiles (Avg Temp / Milk Today w/ "↑12% vs yesterday" / Unactioned Alerts w/ "N critical"). Search cattle + counted status pills (All (N) / Milking (N) / Fever (N) / Heat (N)). Cattle cards (mirror the mock): rounded cow photo, name + #tag, age · belt, a status pill (Heat/Fever danger-tint, Pregnant warn-tint, Milking green-tint), cow temp + trend arrow, and a 3-column info strip whose labels/values switch by status (Milking → Milk Today/Last Milking/Lactation; Pregnant → Due Date/Pregnancy/Last Check; Heat → Heat Detected/Insemination/Heat Stage; Fever → Temp Recorded/Recorded At/Severity). FAB "+" → Add Cattle (placeholder). Highland Farm (0 cattle) shows an empty state. All strings localized (en/hi/bho); farm+cow proper nouns render in Devanagari for hi/bho. |
