...
| Dashboard (Home) — header (logo + farm selector), "Farm Status" section with 3 compact stat cards (Total Cattle/Cows Pregnant/Cows in Heat), Today/This-week tabs (now purely a visual toggle — both tabs render the same list), "Needs Attention" section with 3 tappable "View All" rows (Pending Approvals/Milking Sessions Missed/Critical Alerts), "Cows in heat" + "Cows in gestation" horizontal-scroll rows, Updates list | ✅ Ported — `dashboard_screen.dart` rebuilt to match Home r3 in the HTML: the old 6-tile 2-row grid (which included Pending Approvals/Milkings Missed/Unresolved Alerts tiles, the last with an info-button breakdown + Kajri triage sheet — all now removed) dropped to 3 Farm-Status tiles, and the old per-time schedule rows (vaccination drive/vet visit/milk logging/FMD) were replaced by the 3 Needs-Attention rows; "Pending Approvals" pushes the new `ApprovalsScreen`, "Milking Sessions Missed" and "Critical Alerts" reuse the existing nav-tap handler to open Milk Log / Events; "Cows in heat" cards push `HeatAlertScreen` then open `_FullCycleSheet` via the `openFullCycleFlow()` helper (same path as Events' "View full cycle"), "Cows in gestation" cards deep-link into `_FullCycleSheet` at its gestation step via `openGestationSliderFlow()` — both helpers are public top-level functions in `events_screen.dart` |
| Approvals — full-screen page opened from Home's "Pending Approvals" View All row: back header + title, All/Pending/Approved/Denied filter chips, a "Pending Approvals" section (Deny/Approve button pair per row, resolves in place) on top of a "History" section grouped under date-bucket headers (Today/Yesterday/This Week/Last Week/Last Month, empty buckets skipped) | ✅ Ported — `lib/screens/approvals_screen.dart` (`ApprovalsScreen`); 10-record demo dataset + bucket cutoffs match `APPROVALS_DATA`/`bucketFor()` in `vanix_screens_preview.html` exactly, computed against a fixed reference "today" of 2026-07-22 (never `DateTime.now()`, same pattern as `events_screen.dart`'s hardcoded reminder dates) so both tracks render identical demo content |
...
| Farm detail (cow list) — hero: name + 3-dot kebab (Download Report / Download Critical Report / Share Report with vet / Manage Farm) top row, manager+temp+level left / duration right on the second row, Cattle/Herd Activity tabs. Cattle pane (search + filter sheet Status/Breed/Age, cow cards + kebab), Herd Activity pane (4 activity summary tiles, rumination-style graph w/ anomaly pill, no filter button — see the polish-batch row below for the current chip-driven selection model). Download Report / Download Critical Report open a full-screen Report Preview page (farm name, report-type label, generated-on date, Summary stat tiles — full report adds Critical Alerts + Pending Approvals, critical report shows only Critical Alerts + an italic note — sticky Download button toasts "Report downloaded"); Share Report with vet opens the 3-vet picker sheet and toasts "Report shared with [vet]"; Manage Farm pushes the Farm Management page | ✅ Ported — `lib/screens/farm_detail_screen.dart` (`_farmKebab`, `_openReportPreview`, `_openShareWithVetSheet`, `_farmTempDuration`), `lib/screens/report_preview_screen.dart` (`ReportPreviewScreen`); reuses `FarmMgmtPage` (made public in `lib/screens/account_screen.dart`, was private `_FarmMgmtPage`) |
...
| Polish batch (11 fixes) — **Farm Detail**: bare `more_vert` kebab icon (no circle/border); the mock "0h 48m" duration line removed from the hero; **Report Preview**: header is now title-left + ✕-close-right (was back chevron), Download button is a sticky bottom footer instead of inline content; **Herd Activity**: the funnel filter button was removed entirely (was Cows-only after the Activity category was dropped; the Cows-picker sheet/code is left in place, just unreferenced) — 4 always-visible chips (Rumination/Standing/Resting/Feeding) render under the graph, Rumination toggles independently while Standing/Resting/Feeding are mutually exclusive; with 0–1 selected the graph shows the full 24h day, but selecting exactly 2 zooms the graph to show ONLY their overlapping active-hours window (no data outside it is drawn at all — the lines are sliced to that hour range, not just full-day lines with a shaded overlay) with an "Overlap: HH:00–HH:00 (Nh)" caption, or shows no graph at all plus "No overlap in active hours" if their windows don't intersect; **Add Cattle**: opens as a bottom sheet (leaves the Farm Detail hero peeking through at the top, rounded top corners, dimming backdrop) instead of a full-screen push, background is white (was warm), name field label is now "Name" (was "Cow Name (easily recognisable)"); **Cow Profile**: all timeline dots standardized to `#555555` (was per-event severity color), the "Hide" button removed from expanded timeline cards (tap the card again to collapse — the trailing chevron already flips expand_less/chevron_right), "Change status" row removed from the floating-+ actions sheet (starts directly with "Request a vet visit"; the statusList/statusNote flow code is left in place, just unreferenced), and the root Scaffold + tab-bar background changed from bgwarm to bgcard so there's no warm band between the photo hero and the content below. **Not portable 1:1**: item 8 ("Cattle Detail" tab label) has no Flutter equivalent — `add_cattle_screen.dart` is a single scrolling form, not the HTML's segmented Cattle-Detail/Device-Details tab pair; only the FS string key value itself was left as-is since nothing renders it yet. | ✅ Ported (except the noted tab-label gap) — `farm_detail_screen.dart` (`_farmKebab`, `_fdToggleActivity`, `_ruminationCard`, `_MultiActivityPainter`, `_FdActLine`, `_kFdActMeta`; `_HerdFilterSheet`/`_openHerdFilterSheet` left in place but unreferenced now that the funnel button is gone), `report_preview_screen.dart`, `add_cattle_screen.dart`, `cow_profile_screen.dart` (`_timeline`/`_tlDotColor`, `_timelineCard`, `_mainList`); new i18n keys in `farm_strings.dart` (`overlapWord`, `noOverlapWord`); `acCowName` value changed to "Name" |
