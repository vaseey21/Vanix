...
| Farmer persona (stripped views) | ✅ Ported — persona toggle on login (`_PersonaToggle`), `AppState.persona`/`isFarmer`/`isSingleFarm`, `FarmerDashboardScreen` (Immediate/To-dos tabs, action rows → Events), single-farm farmer opens Farm Detail directly, owner-only Account sections (Farm Mgmt/Cattle Groups/Vet Contacts) + Farm-Detail manager-edit hidden for farmers. **Pending:** milk-edit approval workflow is not yet in the Flutter milk screen, so the farmer's "request → pending" milk state has no Flutter surface to gate yet. |
