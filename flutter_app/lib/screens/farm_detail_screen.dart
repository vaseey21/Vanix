...
class _FarmDetailScreenState extends State<FarmDetailScreen> {
  ...
  void _openDownloadSheet(bool isDark) {
    final lang = _lang;
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      ...
      builder: (ctx) {
        ...
        final textColor = isDark ? Colors.white : VanixColors.textPrimary;
        Widget row(String label, Color color, VoidCallback onTap) => InkWell(
              onTap: onTap,
              child: Container(
                width: double.infinity,
                ...
                child: Row(
                  children: [
                    Icon(Icons.file_download_outlined, size: 17, color: color),
        ...
        return Container(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Container(width: 36, height: 4, decoration: BoxDecoration(color: VanixColors.border, borderRadius: BorderRadius.circular(2))),
              const SizedBox(height: 10),
              Row(children: [
                Expanded(child: Text(FS.t(lang, 'fdReportTitle'), style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: textColor))),
                IconButton(onPressed: () => Navigator.pop(ctx), icon: Icon(Icons.close, size: 16, color: textColor)),
              ]),
              row(FS.t(lang, 'fdDownloadReport'), textColor, () {
                Navigator.pop(ctx);
                _showToast(FS.t(lang, 'fdReportDownloaded'));
              }),
              row(FS.t(lang, 'fdDownloadCritical'), VanixColors.danger, () {
                Navigator.pop(ctx);
                _showToast(FS.t(lang, 'fdCriticalDownloaded'));
              }),
            ],
          ),
        );
  ...
  void _openShareVetSheet(bool isDark) {
    final lang = _lang;
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 10),
              Row(children: [
                Expanded(child: Text(FS.t(lang, 'pickVet'), style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: textColor))),
                IconButton(onPressed: () => Navigator.pop(ctx), icon: Icon(Icons.close, size: 16, color: textColor)),
              ]),
              for (final vet in kVets)
                InkWell(
                  onTap: () {
                    Navigator.pop(ctx);
                    _showToast('${FS.t(lang, 'fdSharedWith')} ${vet.name}');
                  },
                  child: Container(
                    width: double.infinity,
                    constraints: const BoxConstraints(minHeight: 48),
                    ...
                    decoration: BoxDecoration(border: Border.all(color: borderCol), borderRadius: BorderRadius.circular(12)),
                    child: Row(
                      children: [
                        Expanded(child: Text(vet.name, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: textColor))),
  ...
  // ── Range chips + 2x2 stat grid ─────────────────────────────────────────
  Widget _rangeChipRow(bool isDark) {
    final chips = [
      ['today', 'fdRangeToday'],
      ['week', 'fdRangeWeek'],
      ['month', 'fdRangeMonth'],
      ['custom', 'fdRangeCustom'],
    ];
    return SizedBox(
      height: 34,
      child: ListView(
        ...
        children: [
          for (final c in chips)
            Padding(
              ...
              child: _rangeChip(label: FS.t(_lang, c[1]), value: c[0], isDark: isDark),
            ),
        ],
      ),
    );
  ...
  Widget _rangeChip({required String label, required String value, required bool isDark}) {
    final active = _fdRange == value;
    return InkWell(
      onTap: () async {
        if (value == 'custom') {
          await _openCustomRangeSheet(isDark);
        } else {
          setState(() => _fdRange = value);
        }
      },
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: active ? VanixColors.activeBg : (isDark ? VanixColors.darkSecond : VanixColors.bgCard),
          border: Border.all(color: active ? VanixColors.greenDeep : (isDark ? VanixColors.darkBorder : VanixColors.border)),
          borderRadius: BorderRadius.circular(17),
        ),
        child: Text(label,
            style: TextStyle(
              ...
              color: active ? VanixColors.greenInk : (isDark ? Colors.white : VanixColors.textPrimary),
            )),
      ),
    );
  }

  Future<void> _openCustomRangeSheet(bool isDark) async {
    final lang = _lang;
    DateTime? from = _customFrom;
    DateTime? to = _customTo;
    await showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (ctx) {
        ...
        return StatefulBuilder(builder: (ctx, setSheet) {
          Future<void> pick(bool isFrom) async {
            ...
            if (picked != null) setSheet(() => isFrom ? from = picked : to = picked);
          }

          String fmt(DateTime? d) => d == null ? FS.t(lang, 'selectDate') : '${d.day}/${d.month}/${d.year}';
          ...
          return Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 10),
                Row(children: [
                  Expanded(child: Text(FS.t(lang, 'fdCustomRange'), style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: textColor))),
                  IconButton(onPressed: () => Navigator.pop(ctx), icon: Icon(Icons.close, size: 16, color: textColor)),
                ]),
                ...
                InkWell(
                  ...
                  child: Container(
                    ...
                    alignment: AlignmentDirectional.centerStart,
                    decoration: BoxDecoration(border: Border.all(color: borderCol), borderRadius: BorderRadius.circular(12)),
                    child: Text(fmt(from), style: TextStyle(fontSize: 15, color: textColor)),
                  ),
                ),
                const SizedBox(height: 14),
                ...
                InkWell(
                  ...
                  child: Container(
                    ...
                    alignment: AlignmentDirectional.centerStart,
                    decoration: BoxDecoration(border: Border.all(color: borderCol), borderRadius: BorderRadius.circular(12)),
                    child: Text(fmt(to), style: TextStyle(fontSize: 15, color: textColor)),
                  ),
                ),
  ...
  Widget _fdStatGrid(bool isDark) {
    final stats = (_kFdRangeStats[widget.farm.id] ?? const {})[_fdRange] ?? const {'fever': 0, 'heat': 0, 'insem': 0, 'preg': 0};
    return GridView.count(
      ...
      children: [
        _fdTile(label: FS.t(_lang, 'cattleFever'), color: VanixColors.danger, value: stats['fever'] ?? 0, isDark: isDark),
        _fdTile(label: FS.t(_lang, 'cattleHeat'), color: VanixColors.warning, value: stats['heat'] ?? 0, isDark: isDark),
        _fdTile(label: FS.t(_lang, 'insemWord'), color: VanixColors.greenInk, value: stats['insem'] ?? 0, isDark: isDark),
        _fdTile(label: FS.t(_lang, 'statusPregnant'), color: isDark ? Colors.white : VanixColors.textPrimary, value: stats['preg'] ?? 0, isDark: isDark),
      ],
    );
  }

  Widget _fdTile({required String label, required Color color, required int value, required bool isDark}) {
    return Container(
        ...
        children: [
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                Text('$value', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700, color: isDark ? Colors.white : VanixColors.textPrimary)),
                const SizedBox(height: 2),
